...
public class KThread {
	...
	public static void finish() {
		...
		/**
		 * Handle the waiting thread that called join() on this thread,
		 * then clear the willJoin field.
		 */
		if(currentThread.willJoin != null) {
			KThread waitingThread = currentThread.willJoin;
			// currentThread.willJoin = null;
			waitingThread.ready();
		}
	...
	public void join() {
		...
		/**
		* Make sure that join() is called on this thread at most once.
		*/
		Lib.assertTrue(this.willJoin == null);
		...
		/**
		* Assign current thread to the willJoin variable.
		*/
		boolean intStatus = Machine.interrupt().disable(); 
		this.willJoin = currentThread;
		currentThread.sleep();
		Machine.interrupt().restore(intStatus);
    ...
    private static void joinTest1 () {
		KThread child1 = new KThread( new Runnable () {
			public void run() {
				...
				long till = Machine.timer().getTime() + 200;
				while(Machine.timer().getTime() < till) {
					KThread.currentThread().yield();
				}

				System.out.println("child1 ending");
			}
		});
...
public static void hwTest() {
	...
	/**
	 * Record other threads that call join on this thread.
	 */
	public KThread willJoin = null;

	public Boolean inSleepFor = false;
